...
const BlogPage = () => {
  ...
  useEffect(()=>{
    const getPosts = async () => {
      try {
        let { data: {data, pagination} } = await request.get( categoryValue && categoryValue !== "all" ? `post?category=${categoryValue}` : "post" , 
      ...
      <section>
        <div className="container all__posts__container">
          <div className="all__posts__header">
            ...
            <select onChange={ (e) => {selectCategory(e.target.value)} } className="category__select" name="category" id="categoryId">
              <option value="all"> All </option>
              { categoryData?.map((categories, i) =>
              <option key={i} value={categories._id}>{ categories?.name }</option> ) }
            </select>
          </div>
